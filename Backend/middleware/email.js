...
const eventRegistrationMessage = async (email, name, eventName, eventDate, eventLocation, eventWhatsappGroup) => {
    const info = await transporter.sendMail({
        from: `"Slug at Sapthagiri nps university" <${process.env.EMAIL}>`,
        to: email,
        subject: "Registration Confirmed",
        text: "Registration Confirmed", // Plain-text version of the message
        html: Event_Registration_Email_Template.replace("{name}", name).replace("{eventName}", eventName).replace("{eventDate}", eventDate).replace("{eventLocation}", eventLocation).replace("{eventWhatsappGroup}", eventWhatsappGroup) // HTML version of the message
    });
}
